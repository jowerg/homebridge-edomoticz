...
function eDomoticzAccessory(platform, IsScene, status, idx, name, haveDimmer, maxDimLevel, subType, Type, batteryRef, swType, swTypeVal, hwType, eve) {
	...
	this.lastOpened = new Date();					// Last command sent to a Zwave GarageDoor opener
	this.lastOpened.setDate(this.lastOpened.getDate()-1);
	this.isExternal=false;						//  GarageDoor state changed by an external controller (i.e Remote Control, Garage button, etc)
	this.lastTargetState=Characteristic.TargetDoorState.CLOSED;	// Initial Garage door state
	this.tempDoorState;						// Temporary door State
...
eDomoticzAccessory.prototype = {
	...
	setDoorExternal: function() {
 		this.targetDoorState.setValue(this.tempDoorState);
	},
	setFinalDoorExternal: function() {
		this.platform.log("**** setFinalDoorExternal for: " +this.name );

                        Domoticz.deviceStatus(this, function(json) {
				var value;
                                var sArray = Helper.sortByKey(json.result, "Name");
                                sArray.map(function(s) {
				 	value = (s.Status == "Off" || s.Status == "Closed") ? Characteristic.CurrentDoorState.CLOSED : Characteristic.CurrentDoorState.OPEN;
                                }.bind(this));
			this.platform.log("**** setFinalDoorExternal CHECKSTATUS for: " +this.name + " , Value: " + value);
				this.currentDoorState.updateValue(value);
                        }.bind(this));
	...
	setFinalDoorState: function() {
		this.platform.log("**** setFinalDoorState for: " +this.name );

		this.finalDoorState=this.currentDoorState.value;
		this.operating=false;
		var checkstatus=false;

			if (this.finalDoorState==Characteristic.CurrentDoorState.CLOSING){
				this.platform.log("**** setFinalDoorState CLOSING for: " +this.name );
				//this.currentDoorState.setValue(Characteristic.CurrentDoorState.CLOSED);
				checkstatus=true;
			} else if (this.finalDoorState==Characteristic.CurrentDoorState.OPENING){
				this.platform.log("**** setFinalDoorState OPENING for: " +this.name );
				//this.currentDoorState.setValue(Characteristic.CurrentDoorState.OPEN);
				checkstatus=true;
			}

		if (checkstatus==true){
			this.platform.log("**** setFinalDoorState CHECKSTATUS for: " +this.name );
                        Domoticz.deviceStatus(this, function(json) {
				var value;
                                var sArray = Helper.sortByKey(json.result, "Name");
                                sArray.map(function(s) {
			...
			this.platform.log("**** setFinalDoorState CHECKSTATUS for: " +this.name + " , Value: " + value);
				this.currentDoorState.setValue(value);
                        }.bind(this));
		}
        	this.lastOpened = new Date();
        	this.lastOpened.setDate(this.lastOpened.getDate()-1);
	...
	setTargetDoorState: function(powerOn, callback, context) {
		this.platform.log("** SET targetDoorState for " + this.name + ", Value: " + powerOn );

		this.lastTargetState=powerOn;
		this.operating=false;
		var finalExternal=false;
		if (this.isExternal==false){
			this.lastOpened = new Date();
			if (powerOn && (this.currentDoorState.value!=Characteristic.CurrentDoorState.CLOSED)){
				this.operating=true;
				this.currentDoorState.setValue(Characteristic.CurrentDoorState.CLOSING);
			} else if (!powerOn && (this.currentDoorState.value!=Characteristic.CurrentDoorState.OPEN)){
				this.operating=true;
				this.currentDoorState.setValue(Characteristic.CurrentDoorState.OPENING);
			}
		} 
		else 
		{
			this.platform.log("** SET targetDoorState EXTERNAL COMMAND for " + this.name + ", Value: " + powerOn );
			finalExternal=true;
			this.isExternal=false;
		}

		if (this.operating==true){
			//setear un timer de unos 23....
			setTimeout(this.setFinalDoorState.bind(this), 22 * 1000);
		}
	
		Domoticz.updateDeviceStatus(this, "switchlight", {"switchcmd": (powerOn ? "Off" : "On")}, function(success) {
			if (finalExternal==true) {
				setTimeout(this.setFinalDoorExternal.bind(this), 1500);
			}
			callback();
		}.bind(this));
	...
	getCurrentDoorState: function(callback) {
		this.platform.log("** get currentDoorState for " + this.name );

		var preValue=this.currentDoorState.value;
		...
		Domoticz.deviceStatus(this, function(json) {
			...
			sArray.map(function(s) {
				value = (s.Status == "Off" || s.Status == "Closed") ? Characteristic.CurrentDoorState.CLOSED : Characteristic.CurrentDoorState.OPEN;
		this.platform.log("** get currentDoorState for " + this.name + ", Status:" + s.Status);
			}.bind(this));


			var newTarget;
			if ((new Date() - this.lastOpened) >= (35 * 1000)){
				// Mas de 35 Seg desde el ultimo comando...
		this.platform.log("**** get currentDoorState for " + this.name + ", Result:" + value +", Target Value: " + this.targetDoorState.value + " ,Previous Value:"+preValue);
				if (value==Characteristic.CurrentDoorState.CLOSED || value==Characteristic.CurrentDoorState.CLOSING){
					if (this.targetDoorState.value != Characteristic.TargetDoorState.CLOSED){
						this.isExternal=true;
						value=preValue;
						newTarget=Characteristic.TargetDoorState.CLOSED;
					}
				} else if (value==Characteristic.CurrentDoorState.OPEN || value==Characteristic.CurrentDoorState.OPENING){ 
					if (this.targetDoorState.value != Characteristic.TargetDoorState.OPEN){
						this.isExternal=true;
						value=preValue;
						newTarget=Characteristic.TargetDoorState.OPEN;
					}
				}

			} else {
		this.platform.log("**** get currentDoorState2 for " + this.name + ", Result:" + value);
			}
			
			//callback(null, value);
			if (this.isExternal){
		this.platform.log("**** get currentDoorState2 UPDATE TARGET VALUE for " + this.name + ", Result:" + newTarget);
				// vamos a poner un Timer de 500mseg
				this.tempDoorState=newTarget;
				setTimeout(this.setDoorExternal.bind(this), 700);
			}
			callback(null, value);
	...
	getValue: function(callback) {
		Domoticz.deviceStatus(this, function(json) {
			...
			this.platform.log("_Data Received for " + this.name + ": " + value);
			callback(null, value);
		}.bind(this));
	},
	...
	getTemperatureAlternative: function(callback) {
		Domoticz.deviceStatus(this, function(json) {
			...
			value = Helper.fToC(value);
			this.platform.log("2Data Received for " + this.name + ": " + value);
			callback(null, value);
		}.bind(this));
	},
	setPoint: function(setpoint, callback, context) {
		...
		var url = "";
	        var oldpoint;
	        oldpoint = setpoint;
	        setpoint = Helper.cToF(setpoint);
	        setpoint = Helper.fToC(setpoint);
		...
		this.platform.log("Setting thermostat SetPoint to " + setpoint + ", C point: "+ oldpoint+ " url: "+url); 
	...
	this.platform.log("* SetTempOverride for " + this.name + ", Value: " + setpoint );
	...
	getTempOverride: function(callback) {
		Domoticz.deviceStatus(this, function(json) {
			...
			this.platform.log("1Data Received for " + this.name + ": " + value);
			callback(null, value);
		}.bind(this));
	},
			...
			case this.swTypeVal == Constants.DeviceTypeSwitch || this.swTypeVal == Constants.DeviceTypePushOn:
				{
					...
					if (this.name.indexOf("Fan") > -1) {
						...
						callback(characteristic, message.nvalue);
                                        } else if (this.name.indexOf("Garage") > -1) {
						var characteristic = this.getService(Service.GarageDoorOpener).getCharacteristic(Characteristic.CurrentDoorState);
						callback(characteristic, message.nvalue);
					} else {
	...
	getServices: function() {
		...
		if (this.isSwitch) {
			...
			case this.swTypeVal == Constants.DeviceTypeSwitch || this.swTypeVal == Constants.DeviceTypePushOn:
				{
					this.platform.log("** DEVICE TYPE  SWITCH " + this.name );
					var service = false;
					if (this.name.indexOf("Fan") > -1) {
						...
						this.targetDoorState.on('set', this.setTargetDoorState.bind(this));
                                                this.targetDoorState.on('get', this.getTargetDoorState.bind(this));
						this.currentDoorState=service.getCharacteristic(Characteristic.CurrentDoorState);
						this.currentDoorState.on('get', this.getCurrentDoorState.bind(this));
					} else {
			...
			case this.swTypeVal == Constants.DeviceTypeContact:
				{
					this.platform.log("** DEVICE TYPE  CONTACT " + this.name );
			...
			case this.swTypeVal == Constants.DeviceTypeSmoke:
				{
					this.platform.log("** DEVICE TYPE  SMOKE " + this.name );
			...
			case this.swTypeVal == Constants.DeviceTypeMedia:
				{
					this.platform.log("** DEVICE TYPE  DIMMER " + this.name );
			...
			case this.swTypeVal == Constants.DeviceTypeMotion:
				{
					this.platform.log("** DEVICE TYPE  MOTION " + this.name );
			...
			case this.swTypeVal == Constants.DeviceTypeDoorLock:
				{
					this.platform.log("** DEVICE TYPE  DOOR LOCK " + this.name );
					var lockService;
					if (this.name.indexOf("Garage") > -1) {
                                                lockService = new Service.GarageDoorOpener(this.name);
                                                lockService.setCharacteristic(Characteristic.TargetDoorState, Characteristic.TargetDoorState.CLOSED);
                                                lockService.setCharacteristic(Characteristic.CurrentDoorState, Characteristic.CurrentDoorState.CLOSED);

                                                this.targetDoorState=lockService.getCharacteristic(Characteristic.TargetDoorState);
                                                this.targetDoorState.on('set', this.setTargetDoorState.bind(this));
                                                this.targetDoorState.on('get', this.getTargetDoorState.bind(this));
                                                this.currentDoorState=lockService.getCharacteristic(Characteristic.CurrentDoorState);
                                                this.currentDoorState.on('get', this.getCurrentDoorState.bind(this));
                                        } else { 
			...
			case this.Type == "Heating" || this.Type == "Thermostat":
				{
					...
					HeatingDeviceService.getCharacteristic(Characteristic.CurrentHeatingCoolingState).on('get', this.getStateTemp.bind(this));
					HeatingDeviceService.getCharacteristic(Characteristic.TargetHeatingCoolingState).on('get', this.getStateTemp.bind(this));
