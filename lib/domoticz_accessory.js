...
function eDomoticzAccessory(platform, IsScene, status, idx, name, haveDimmer, maxDimLevel, subType, Type, batteryRef, swType, swTypeVal, hwType, eve) {
	...
	this.lastTargetState=Characteristic.TargetDoorState.OPEN;
	this.tempDoorState;
...
eDomoticzAccessory.prototype = {
	...
	setTargetDoorState: function(powerOn, callback, context) {
		...
		if (this.operating==true){
			...
			setTimeout(this.setFinalDoorState.bind(this), 23 * 1000);
		}
	...
	getDoorState2: function(callback) {
	...
	setPoint: function(setpoint, callback, context) {
		...
		if (this.subType == "SetPoint") {
			url = this.platform.apiBaseURL + "type=command&param=setsetpoint&idx=" + this.idx;
			url = url + "&setpoint=" + setpoint;
			...
			case this.swTypeVal == Constants.DeviceTypeSwitch || this.swTypeVal == Constants.DeviceTypePushOn:
				{
					...
					if (this.name.indexOf("Fan") > -1) {
						...
						this.currentDoorState.on('get', this.getDoorState2.bind(this));
					} else {
			...
			case this.swTypeVal == Constants.DeviceTypeDoorLock:
				{
					...
					if (this.name.indexOf("Garage") > -1) {
                                                ...
                                                lockService.setCharacteristic(Characteristic.TargetDoorState, Characteristic.TargetDoorState.OPEN);
                                                lockService.setCharacteristic(Characteristic.CurrentDoorState, Characteristic.CurrentDoorState.OPEN);
